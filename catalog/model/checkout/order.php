...
public function addOrder(array $data): int {
    ...
    // Save referral details into oc_referral (if available)
if (!empty($data['referrer_name']) || !empty($data['referrer_mobile'])) {
    $this->db->query("INSERT INTO `" . DB_PREFIX . "referral` SET 
        ...
        `referrer_mobile` = '" . $this->db->escape($data['referrer_mobile'] ?? '') . "',
        `date_added` = NOW()");
}

...
document.addEventListener('DOMContentLoaded', function() {
  var dateInputs = document.querySelectorAll('.custom-date');
  ...
  dateInputs.forEach(function(input) {
    ...
    // Prevent any non-numeric and non-slash key, enforce length, and single slash
    input.addEventListener('keydown', function(e) {
      // Allow: backspace, tab, delete, arrow keys
      if ([8,9,46,37,39].indexOf(e.keyCode) !== -1) return;

      // Limit length to 5
      if (input.value.length >= 5 && ![8,46,37,39].includes(e.keyCode)) e.preventDefault();
      // Only digits or one slash (which JS auto-inserts)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let swiperThumbs = new Swiper(".peptoply-mySwiper-side", {
      spaceBetween: 25,
      slidesPerView: getSlidesPerView(), // Dynamically set slidesPerView
      freeMode: true,
      watchSlidesProgress: true,
      watchSlidesVisibility: true,
      direction: getDirection(), // Dynamically set direction
      breakpoints: {
        1200: { direction: "vertical", slidesPerView: 3 },
        750: { direction: "horizontal", slidesPerView: 2 },
        0: { direction: "horizontal", slidesPerView: 1 },
      },
    });

    let swiperMain = new Swiper(".peptoply-mySwiper-main", {
      loop: true,
      spaceBetween: 10,
      navigation: {
        nextEl: ".peptoply-swiper-button-next",
        prevEl: ".peptoply-swiper-button-prev",
      },
      thumbs: { swiper: swiperThumbs },
      on: {
        slideChangeTransitionEnd: function () {
          let activeIndex = this.realIndex;
          swiperThumbs.slideTo(activeIndex); // Sync thumb with main slider
        },
      },
    });

    return { swiperThumbs, swiperMain };
  }
  ...
  let { swiperThumbs, swiperMain } = initializeSwipers();
  ...
  window.addEventListener("resize", () => {
    ...
    resizeTimer = setTimeout(() => {
      ...
      ({ swiperThumbs, swiperMain } = initializeSwipers());
    }, 300);
  });

       window.addEventListener("scroll", function () {
      let header = document.querySelector(".peptoply-header"); // Change to your element
  ...
  var swiper3 = new Swiper('.peptoply-product-swiper', {
    slidesPerView: 1,
    spaceBetween: 20,
    loop: false,
    navigation: {

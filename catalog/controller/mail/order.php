...
if ($order_info) {
    $this->log->write('Mail::index | order=' . $order_id . ' | old_status=' . $order_info['order_status_id'] . ' | new_status=' . $order_status_id . ' | notify=' . (int)$notify);

    if (!$order_info['order_status_id'] && $order_status_id) {
        $this->log->write('Mail::index | calling add()');
	...
	/**
	 * Send SMS helper — reusable for both add() and history()
	 *
	 * @param array<string, mixed> $order_info
	 * @param int                  $order_status_id
	 * @param string               $sms_flag  — unique comment flag to prevent duplicates
	 *
	 * @return void
	 */
	private function sendSms(array $order_info, int $order_status_id, string $sms_flag): void {
		$mobile  = $order_info['telephone'];
		$orderId = (int)$order_info['order_id'];

		if (!$orderId) {
			$this->log->write('Fast2SMS: Skipped — order ID is 0');
			return;
		}

		// Prevent duplicate SMS using DB flag
		$sms_check = $this->db->query("
			SELECT COUNT(*) AS total
			FROM `" . DB_PREFIX . "order_history`
			WHERE order_id = '" . $orderId . "'
			AND comment = '" . $this->db->escape($sms_flag) . "'
		");

		if ($sms_check->row['total']) {
			return;
		}

		// Clean mobile number
		$mobile_digits = preg_replace('/\D/', '', $mobile);
		$mobile_digits = ltrim($mobile_digits, '0');

		if (strlen($mobile_digits) !== 10) {
			$this->log->write('Fast2SMS: Invalid mobile - ' . $mobile);
			return;
		}

		$apiKey = $this->config->get('fast2sms_api_key')
			?: (defined('FAST2SMS_API_KEY') ? FAST2SMS_API_KEY : '');

		if (!$apiKey) {
			$this->log->write('Fast2SMS: API key not configured');
			return;
		}
		...
		if ($error) {
			$this->log->write('Fast2SMS CURL Error: ' . $error);
			return;
		}
		...
		if (!empty($result['return']) && $result['return'] === true) {
			$this->db->query("
				...
				SET order_id = '" . $orderId . "',
					...
					notify = '0',
					comment = '" . $this->db->escape($sms_flag) . "',
					date_added = NOW()
			");
			$this->log->write('Fast2SMS: SMS sent | Order #' . $orderId . ' | Flag: ' . $sms_flag);
		} else {
			$this->log->write('Fast2SMS FAILED: ' . $response);
		}
	}
	...
	public function add(array $order_info, int $order_status_id, string $comment, bool $notify): void {
		...
		$is_demo_email = str_ends_with(strtolower(trim($order_info['email'])), '@peptoply-demo.com');

		if (!$is_demo_email && $this->config->get('config_mail_engine')) {
			$mail_option = [
			...
			$mail->send();
		} elseif ($is_demo_email) {
			$this->log->write('Mail: Skipped email for demo account | Order #' . $order_info['order_id'] . ' | ' . $order_info['email']);
		}

		// Send SMS for every order regardless of email
		$this->sendSms($order_info, $order_status_id, 'SMS_SENT_ORDER_PLACED');
	}
	...
	public function history(array $order_info, int $order_status_id, string $comment, bool $notify): void {
		...
		if (!$is_demo_email && $this->config->get('config_mail_engine')) {
			...
			$this->log->write('Mail: Skipped history email for demo account | Order #' . $order_info['order_id'] . ' | ' . $order_info['email']);
		}

		// Send SMS on every status update regardless of email
		// $this->sendSms($order_info, $order_status_id, 'SMS_SENT_ORDER_STATUS_' . $order_status_id);
	}

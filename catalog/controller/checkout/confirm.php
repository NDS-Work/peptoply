...
class Confirm extends \Opencart\System\Engine\Controller {
	...
	public function index(): string {
		...
		// ═══════════════════════════════════════════════════════════════
		// 🔴 BUG FIX #2: Validate order matches current session state
		// ═══════════════════════════════════════════════════════════════
		// Defensive check to detect if order data is stale (payment method
		// or GST changed since order was created) and force regeneration.
		// This is a safety net in case order_id clearing (Fix #1) missed something.
		if ($order_id && $order_info) {
			$current_payment_code = $this->session->data['payment_method']['code'] ?? '';
			$order_needs_refresh = false;

			// Check if payment method has changed
			if (isset($order_info['payment_method']) &&
				is_array($order_info['payment_method']) &&
				isset($order_info['payment_method']['code'])) {

				$order_payment_code = $order_info['payment_method']['code'];

				if ($current_payment_code !== $order_payment_code) {
					$order_needs_refresh = true;
				}
			}

			// Check if GST has changed (if payment method unchanged)
			if (!$order_needs_refresh) {
				$current_gst = $this->session->data['order_gst'] ?? '';
				$order_gst = '';

				// Extract GST from order comment if it exists
				if (!empty($order_info['comment']) && strpos($order_info['comment'], 'GST Number:') !== false) {
					preg_match('/GST Number:\s*(.+?)(?:\n|$)/', $order_info['comment'], $matches);
					$order_gst = trim($matches[1] ?? '');
				}

				if ($current_gst !== $order_gst) {
					$order_needs_refresh = true;
				}
			}

			// If order is stale, invalidate it and create a new one
			if ($order_needs_refresh) {
				unset($this->session->data['order_id']);
				$order_id = 0;
				$order_info = null;
...
// Add GST to order comment only if active in session
if (!empty($this->session->data['order_gst'])) {

    $gst_number = $this->session->data['order_gst'];
    $gst_line = "GST Number: " . $gst_number;

    // prevent duplicate append
    if (empty($order_data['comment']) || strpos($order_data['comment'], $gst_number) === false) {

        if (!empty($order_data['comment'])) {
            $order_data['comment'] .= "\n" . $gst_line;
        } else {
            $order_data['comment'] = $gst_line;
        }
    }
}
...
if (!$order_id) {
    ...
    $this->model_checkout_order->editOrder($order_id, $order_data);
						...
						if (isset($this->session->data['referral']) && isset($this->session->data['order_id'])) {
    $referral = $this->session->data['referral'];

    $order_id = $this->session->data['order_id'];
    $customer_id = $this->session->data['customer']['customer_id'] ?? 0;

    $this->db->query("INSERT INTO `" . DB_PREFIX . "referral` SET 
        `customer_id` = '" . (int)$customer_id . "',
        `referrer_id` = '0',
        `referrer_name` = '" . $this->db->escape($referral['referrer_name']) . "',
        `referrer_mobile` = '" . $this->db->escape($referral['referrer_mobile']) . "',

...
class Product extends \Opencart\System\Engine\Controller {
	...
	public function index(): ?\Opencart\System\Engine\Action {
		...
		// Clear any stored options from session for this product
		if (isset($this->session->data['selected_options'])) {
			unset($this->session->data['selected_options']);
		}
    ...
    'default_value_id'     => $default_option_value_id
] + $option;
				}
			}
			...
			$data['features_section'] = $this->load->view('custom/features', $data);
			$data['termi_tuff_section'] = $this->load->view('custom/termi_tuff', $data);
			// Send cart product IDs to view
			$cart_products = [];
    ...
    try {
        ...
        $json['debug'][] = 'product_id=' . $product_id;
        ...
        // Base = special if present, else price (so UI mirrors what customer sees)
        $base = (float)($product_info['special'] ?: $product_info['price']);
        $price_from = $product_info['special'] ? 'special' : 'price';
        $json['debug'][] = "base={$base} from={$price_from}";

        // IMPORTANT: get options from master if variant
        $master_id = $product_info['master_id'] ? (int)$product_info['master_id'] : $product_id;
        $product_options = $this->model_catalog_product->getOptions($master_id);

        $posted_options = $this->request->post['option'] ?? [];
        $json['debug'][] = 'posted_options=' . json_encode($posted_options);

        $delta = 0.0;

        foreach ($product_options as $option) {
            if (empty($option['product_option_value'])) continue;

            $poid = $option['product_option_id'];

            if (!isset($posted_options[$poid])) continue;

            $sel = $posted_options[$poid];

            foreach ($option['product_option_value'] as $ov) {
                $is_selected = is_array($sel)
                    ? in_array($ov['product_option_value_id'], $sel)
                    : ((string)$sel === (string)$ov['product_option_value_id']);

                if ($is_selected) {
                    $change = (float)$ov['price'];
                    if ($ov['price_prefix'] === '-') $change = -$change;
                    $delta += $change;
                    $json['debug'][] = "opt {$poid} val {$ov['product_option_value_id']} change {$ov['price_prefix']}{$ov['price']} delta_now={$delta}";
                }
            }
        }

        $final = $base + $delta;
        $json['debug'][] = "final={$final}";

        // Format final for current currency + tax
        $json['price'] = $this->currency->format(
            $this->tax->calculate($final, (int)$product_info['tax_class_id'], $this->config->get('config_tax')),
            $this->session->data['currency']
        );

        // Also provide both "original" branches so twig can update old/new cleanly
        $with_delta_price   = (float)$product_info['price']   + $delta;
        $with_delta_special = $product_info['special'] !== null && $product_info['special'] !== ''
                                ? (float)$product_info['special'] + $delta : null;

        $json['original_price'] = $this->currency->format(
            $this->tax->calculate($with_delta_price, (int)$product_info['tax_class_id'], $this->config->get('config_tax')),
            $this->session->data['currency']
        );

        $json['original_special'] = $with_delta_special !== null ? $this->currency->format(
            $this->tax->calculate($with_delta_special, (int)$product_info['tax_class_id'], $this->config->get('config_tax')),
            $this->session->data['currency']
        ) : '';

        // Write server-side log line
        if (isset($this->log)) {
            $this->log->write('[updatePrice] ' . implode(' | ', $json['debug']));
        } else {
            error_log('[updatePrice] ' . implode(' | ', $json['debug']));
        }
    } catch (\Throwable $e) {
        $json['error'] = $e->getMessage();
        if (isset($this->log)) {
            $this->log->write('[updatePrice][ERROR] ' . $e->getMessage());
        } else {
            error_log('[updatePrice][ERROR] ' . $e->getMessage());
        }
    }

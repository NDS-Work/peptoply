...
protected function getList(): string {
    ...
    foreach ($results as $result) {
        // Decode custom fields and get GST
// Decode custom fields safely and extract GST
if (isset($result['custom_field'])) {
    if (is_string($result['custom_field'])) {
...
$gst_number = $custom_field['31'] ?? 'not found';
        ...
        $formatted_address = str_replace(
            ...
            preg_replace(
                ["/\\s\\s+/", "/\r\r+/", "/\n\n+/"],
                '<br/>',
                trim(str_replace($find, $replace, $result['address_format']))
            )
        );

        // Push clean array to addresses
        $data['addresses'][] = [

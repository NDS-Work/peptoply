...
class Edit extends \Opencart\System\Engine\Controller {
    ...
    if ($custom_field['location'] == 'account') {
        ...
        // 🔸 Make GST Number (ID 33) optional, even if marked required in admin
        if ($custom_field['custom_field_id'] == 33) {
            continue; // skip "required" check for GST Number
        }

        if ($custom_field['required'] && empty($post_info['custom_field'][$custom_field['custom_field_id']])) {
            $json['error']['custom_field_' . $custom_field['custom_field_id']] =
                sprintf($this->language->get('error_custom_field'), $custom_field['name']);
        } elseif (
            $custom_field['type'] == 'text'
            && !empty($custom_field['validation'])
            && !oc_validate_regex($post_info['custom_field'][$custom_field['custom_field_id']], $custom_field['validation'])
        ) {
            $json['error']['custom_field_' . $custom_field['custom_field_id']] =
                sprintf($this->language->get('error_regex'), $custom_field['name']);
        }
    }
}

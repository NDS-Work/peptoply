...
class MultiShipping extends \Opencart\System\Engine\Model {
			...
			if ($status) {
				...
				if ((string)$cost != '') {
					$quote_data[$method_code] = [
						...
						'cost' => ($method['rate_type'] == 'item' ? $cost * $items : $cost),
						'tax_class_id' => $method['tax_class_id'],
						'text' => ($method['rate_type'] == 'item' ? $this->formatTitle($method, $cost * $items) : $this->formatTitle($method, $cost)),
					];
				}
			}
		} 
	...
	private function isMethodApplicable($method, $subtotal, $cart_weight) {
		return ($method['rate_type'] == 'flat' && $subtotal >= (float)$method['total']) ||
			   ...
			   ($method['rate_type'] == 'item' && $subtotal >= (float)$method['total']) ||
	...
	private function calculateCost($method, $subtotal, $cart_weight) {
		switch ($method['rate_type']) {
			...
			case 'price':
				return $this->calculatePriceCost($method['price_rate'], $subtotal);
			case 'weight':
				return $this->calculateWeightCost($method['weight_rate'], $cart_weight);
			case 'item':
				return $method['item_rate'];
			default:
				return 0;
		}
	}

	private function calculatePriceCost($method_rate, $cart_subtotal) {
	...
	private function calculateWeightCost($method_rate, $cart_weight) {
		$rates = explode(',', $method_rate);
		$cost = '';

...
class Blog extends \Opencart\System\Engine\Controller {
		...
		if ($results) {
			foreach ($results as $result) {
				if ($result['image']) {
					...
					$image = 'catalog/peptoply/placeholder/blog-placeholder.png';
				}
	...
	'description' => oc_substr(
		trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))),
		0,
		$this->config->get('config_article_description_length')
	) . '..',
	...
	'filter_author' => $this->url->link(
		'cms/blog',
		'language=' . $this->config->get('config_language') . '&author=' . urlencode($result['author'])
	),

	
	'href' => $this->url->link(
		'cms/blog.info',
		'language=' . $this->config->get('config_language') . '&article_id=' . $result['article_id']
	)
];
			}
